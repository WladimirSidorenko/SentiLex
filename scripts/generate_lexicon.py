...
SYNRELS = set(["has_pertainym", "is_related_to", "entails", "is_entailed_by",
               "has_hyponym", "has_hypernym"])
...
def _es_train_binary(a_clf_pos, a_clf_neg, a_pos, a_neg, a_neut):
    ...
    print("Fitting parameters of positive-vs-all model... ",
          end="", file=sys.stderr)
    a_clf_pos.fit(instances, pos_classes)
    print("done", file=sys.stderr)
    del pos_classes[:]
    # train negative-vs-all classifier
    print("Fitting parameters of negative-vs-all model... ",
          end="", file=sys.stderr)
...
def _es_generate_candidates(a_germanet, a_synid2tfidf, a_seeds,
...
def _es_expand_sets_binary(a_germanet, a_synid2tfidf, a_clf_pos, a_clf_neg,
    ...
    @param a_pos - set of synset id's and their tf/idf vectors that have
      positive polarity
    @param a_neg - set of synset id's and their tf/idf vectors that have
      negative polarity
    @param a_neut - set of synset id's and their tf/idf vectors that have
      neutral polarity
    ...
    ret = False
    if VERBOSE:
        print("_es_expand_sets_binary: active_synsets = ",
              a_N - N, file=sys.stderr)
        print("_es_expand_sets_binary: N = ", N, file=sys.stderr)
    pos_candidates = set()
    neg_candidates = set()
    # obtain potential candidates
    _es_generate_candidates(a_germanet, a_synid2tfidf,
                            a_pos, pos_candidates, neg_candidates)
    _es_generate_candidates(a_germanet, a_synid2tfidf, a_neg,
    ...
    if pos_candidates or neg_candidates:
        ...
        if VERBOSE:
            print("_es_expand_sets_binary: no candidates generated",
                  file=sys.stderr)
        return False
    # obtain predictions for the potential positive terms
    if pos_candidates:
        pos_pred = a_clf_pos.predict([_es_flatten(iitem)
                                      for _, iitem in pos_candidates])
        neg_pred = a_clf_neg.predict([_es_flatten(iitem)
                                      for _, iitem in pos_candidates])
        # obtain new positive terms based on the made predictions
        new_pos = set(iitem
                      for iitem, ipos, ineg in zip(pos_candidates,
                                                   pos_pred, neg_pred)
    ...
    # reaching the limit
    if VERBOSE:
        print("_es_expand_sets_binary: pos_candidates = ",
              len(pos_candidates), file=sys.stderr)
        print("_es_expand_sets_binary: new_pos = ",
              len(new_pos), file=sys.stderr)
        print("_es_expand_sets_binary: neg_candidates = ",
              len(neg_candidates), file=sys.stderr)
        print("_es_expand_sets_binary: new_neg = ",
              len(new_neg), file=sys.stderr)
    ...
    if N < new_pterms:
        ...
        if VERBOSE:
            print("_es_expand_sets_binary: p_prop =",
                  int(floor(N * p_prop)), file=sys.stderr)
            print("_es_expand_sets_binary: n_prop =",
                  n_prop, file=sys.stderr)
...
def esuli_sebastiani(a_germanet, a_N, a_clf_type, a_clf_arity,
    ...
    if a_clf_type == SVM:
        ...
        raise RuntimeError(
            "Unknown classifier type: '{:s}'".format(a_clf_type))
    i = 0
...
def main(a_argv):
    ...
    subparser_esuli.add_argument("--clf-type",
                                 help="type of classifier to use in ensemble",
                                 choices=[ROCCHIO, SVM], default=SVM)
    subparser_esuli.add_argument("--clf-arity",
                                 help="classifier's arity",
    ...
    subparser_esuli.add_argument("--form2lemma", "-l",
                                 ...
                                 type=str)
    subparser_esuli.add_argument("N",
    ...
    if N > 1:
        ...
        if args.dmethod == ESULI:
            ...
            takamura(igermanet, N, getattr(args, CC_FILE),
                     POS_SET, NEG_SET, NEUT_SET,
                     a_plot=args.plot or None)
        print("Expanding seed sets... done", file=sys.stderr)

    for iclass, iset in ((POSITIVE, POS_SET),
                         (NEGATIVE, NEG_SET), (NEUTRAL, NEUT_SET)):
